...
namespace ThrusterTest.UserControls
{
    public partial class ucGraphs : UserControl
    {
        ...
        private Dictionary<string, Dictionary<string, Series>> chartSeriesDictionary = new Dictionary<string, Dictionary<string, Series>>();
        ...
        {
            chartSeriesDictionary.TryAdd("chart1", new Dictionary<string, Series>());
            chartSeriesDictionary.TryAdd("chart2", new Dictionary<string, Series>());
            chartSeriesDictionary.TryAdd("chart3", new Dictionary<string, Series>());
            chartSeriesDictionary.TryAdd("chart4", new Dictionary<string, Series>());
                    ...
                    using (var reader = new StreamReader(fileInfo.FullName))
                    {
                        ...
                        while ((line = reader.ReadLine()) != null)
                        {
                            ...
                            if (parts.Length >= 4)
                            {
                                ...
                                if (double.TryParse(parts[2], out xValue) && double.TryParse(parts[3], out yValue))
                                {
                                    var chart = Controls.Find(chartName, true).FirstOrDefault() as Chart;
                                    if (chart != null && IsSeriesInChart(chartName, seriesName))
                                    {
        ...
        private bool IsSeriesInChart(string chartName, string seriesName)
        {
            switch (chartName)
            {
                case "chart1":
                    return seriesName == "T1" || seriesName == "T2" || seriesName == "T3" || seriesName == "T4";
                case "chart2":
                    return seriesName == "A1" || seriesName == "A2" || seriesName == "A3" || seriesName == "A4";
                case "chart3":
                    return seriesName == "Actual Velocity" || seriesName == "Desired Velocity";
                case "chart4":
                    return seriesName == "Actual Angular" || seriesName == "Desired Angular";
                default:
                    return false;
            }
        }
        ...
        private void exportToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                folderDialog.Description = "Select folder to save CSV file";
                if (folderDialog.ShowDialog() == DialogResult.OK && !string.IsNullOrWhiteSpace(folderDialog.SelectedPath))
                {
                    var fileName = Interaction.InputBox("Enter file name (without extension):", "Save CSV File", "Chart_Data");
                    if (!string.IsNullOrWhiteSpace(fileName))
                    {
                        var file = new FileInfo(Path.Combine(folderDialog.SelectedPath, $"{fileName}.csv"));

                        using (var writer = new StreamWriter(file.FullName))
                        {
                            foreach (var chartName in new[] { "chart1", "chart2", "chart3", "chart4" })
                            {
                                var chart = Controls.Find(chartName, true).FirstOrDefault() as Chart;
                                if (chart != null)
                                {
                                    var seriesNames = GetSeriesNamesForChart(chartName);
                                    foreach (var seriesName in seriesNames)
                                    {
                                        var series = chart.Series.FindByName(seriesName);
                                        if (series != null)
                                        {
                                            ...
                                            {
                                                writer.WriteLine($"{chartName},{seriesName},{point.XValue},{point.YValues[0]}");
                                            }
                                        }
                                    }
                                }
        ...
        private string[] GetSeriesNamesForChart(string chartName)
        {
            switch (chartName)
            {
                case "chart1":
                ...
                case "chart3":
                    return new string[] { "Actual Velocity", "Desired Velocity" };
                case "chart4":
                    return new string[] { "Actual Angular", "Desired Angular" };
                default:
                    return new string[] { "" };
            }
        }
